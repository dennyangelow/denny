...
export function validateName(name: string): string {
  const v = name.trim()
  if (!v) return 'Моля, въведи твоето имe'
...
export function serverValidate(params: {
  ...
  const DISPOSABLE = new Set([
    ...
    'throwaway.email', 'maildrop.cc', 'trashmail.com', 'trashmail.me',
    'trashmail.net', '10minutemail.com', 'mailsac.com', 'spam4.me',
  ])
  ...
  for (const p of FAKE) {
    if (p.test(local)) return { ok: false, error: 'Въведи реален имейл', field: 'email' }
  }
