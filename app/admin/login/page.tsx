...
function LoginContent() {
  ...
  useEffect(() => {
    fetch('/api/admin/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: '__check__' }) })
      .then(r => r.json()).then(d => { if (d.ok) setNoSecret(true) }).catch(()=>{})
  }, [])

  useEffect(() => { if (noSecret) router.replace(from) }, [noSecret, from, router])

  // Countdown timer за lockout
  useEffect(() => {
    if (lockout <= 0) return
    const t = setInterval(() => setLockout(l => { if (l <= 1) clearInterval(t); return l - 1 }), 1000)
      ...
      <div style={{ background:'#fff', borderRadius:24, padding:'44px 40px', width:'100%', maxWidth:400, textAlign:'center', boxShadow:'0 24px 80px rgba(0,0,0,0.5)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ position:'relative' }}>
            <input
              type="password" placeholder="Въведи паролата" value={password}
              onChange={e => setPassword(e.target.value)} autoFocus disabled={lockout > 0}
              style={{ width:'100%', padding:'14px 16px', paddingLeft:44, border:`1.5px solid ${error ? '#ef4444' : '#e5e7eb'}`, borderRadius:12, fontSize:15, outline:'none', fontFamily:'inherit', color:'#111', background:'#fafafa', transition:'border-color .2s', boxSizing:'border-box' }}
          ...
          {error && (
            <div style={{ background:'#fee2e2', border:'1px solid #fca5a5', borderRadius:10, padding:'10px 14px', fontSize:13, color:'#991b1b', textAlign:'left', display:'flex', gap:8, alignItems:'flex-start' }}>
              ...
              <div>
                <div>{error}</div>
                {lockout > 0 && <div style={{ fontSize:12, marginTop:4, fontWeight:700 }}>Изчакай: {Math.floor(lockout/60)}:{String(lockout%60).padStart(2,'0')}</div>}
              </div>
            </div>
          )}

          <button type="submit" disabled={loading || !password.trim() || lockout > 0}
            style={{ background: (!password.trim() || loading || lockout > 0) ? '#d1d5db' : 'linear-gradient(135deg,#2d6a4f,#16a34a)', color:'#fff', border:'none', borderRadius:12, padding:'14px', fontWeight:800, fontSize:16, cursor: (!password.trim() || loading || lockout > 0) ? 'default' : 'pointer', fontFamily:'inherit', transition:'all .2s' }}>
            {loading ? 'Влизане...' : lockout > 0 ? `Изчакай ${lockout}с` : 'Влез в Admin →'}
          </button>
        </form>

        <p style={{ fontSize:12, color:'#9ca3af', marginTop:20, lineHeight:1.5 }}>
          Паролата се задава чрез <code style={{ background:'#f3f4f6', padding:'2px 6px', borderRadius:4, fontSize:11 }}>ADMIN_SECRET</code> в Vercel
        </p>
      </div>
      <style>{`
