'use client'
// app/admin/components/ContentTab.tsx — v2 с наръчници + fixed race condition
...
export function ContentTab() {
  ...
  // FIX: use ref to prevent double-load in StrictMode and race condition
  ...
  return (
    <div style={{padding:'28px 32px'}}>
      {toast && <div style={{position:'fixed',top:20,right:20,background:'#0d2b1d',color:'#4ade80',padding:'12px 20px',borderRadius:12,fontWeight:700,fontSize:14,zIndex:999,boxShadow:'0 4px 20px rgba(0,0,0,.25)'}}>{toast}</div>}
      ...
      {sub==='affiliate' && (<>
        {editing!==null ? (
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:16}}>
            ...
            {aff.length===0 && <div style={{color:'#6b7280',padding:40,textAlign:'center'}}>Няма афилиейт продукти</div>}
          </div>
        )}
      </>)}
      ...
      {sub==='own' && (<>
        {editing!==null ? (
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:16}}>
            {own.map(p=>(
              <div key={p.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:16,overflow:'hidden',opacity:p.active?1:0.55}}>
                ...
                <div style={{padding:16}}>
                  ...
                  <p style={{fontSize:13,color:'#374151',marginBottom:10}}><strong>{Number(p.price).toFixed(2)} €</strong> / {p.unit} · Наличност: {p.stock}</p>
                  <div style={{display:'flex',gap:8}}>
            ...
            {own.length===0 && <div style={{color:'#6b7280',padding:40,textAlign:'center'}}>Няма собствени продукти</div>}
          </div>
        )}
      </>)}
      ...
      {sub==='links' && (<>
        {editing!==null ? (
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            ...
            {links.length===0 && <div style={{color:'#6b7280',padding:40,textAlign:'center'}}>Няма категорийни линкове</div>}
          </div>
        )}
      </>)}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {nars.map(n=>(
              <div key={n.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:16,padding:'18px 20px',display:'flex',gap:16,alignItems:'flex-start',opacity:n.active?1:0.55}}>
                {n.cover_image_url && <img src={n.cover_image_url} alt={n.title} style={{width:72,height:96,objectFit:'cover',borderRadius:8,flexShrink:0}}/>}
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:'flex',gap:16,fontSize:12,color:'#9ca3af'}}>
                    ...
                    <a href={`/naruchnik/${n.slug}`} target="_blank" style={{color:'#2d6a4f',textDecoration:'none',fontWeight:600}}>↗ Преглед</a>
                    <span style={{fontFamily:'monospace',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:200}}>{n.pdf_url}</span>
                  </div>
                </div>
        ...
        <div>
          ...
          {form.bullets.map((b,i)=>(
            <div key={i} style={{display:'flex',gap:8,marginBottom:6}}>
              ...
              <button onClick={()=>setForm({...form,bullets:form.bullets.filter((_,j)=>j!==i)})} style={{background:'#fee2e2',border:'none',borderRadius:8,padding:'0 10px',cursor:'pointer',color:'#991b1b',flexShrink:0}}>✕</button>
            </div>
          ))}
          <button onClick={()=>setForm({...form,bullets:[...form.bullets,'']})} style={{background:'#f0fdf4',border:'1px dashed #86efac',borderRadius:8,padding:'7px 16px',cursor:'pointer',color:'#166534',fontWeight:600,fontSize:13,fontFamily:'inherit',marginTop:4}}>+ Добави</button>
        </div>
  ...
  return (
    <div className="form-card">
      ...
      <div className="cf">
        ...
        <div className="form-row">
          <div><label>Цена (€)</label><input type="number" step="0.01" value={form.price} onChange={e=>setForm({...form,price:+e.target.value})}/></div>
          <div><label>Стара цена (€)</label><input type="number" step="0.01" value={form.compare_price} onChange={e=>setForm({...form,compare_price:+e.target.value})}/></div>
        </div>
        <div className="form-row">
          <div><label>Наличност</label><input type="number" value={form.stock} onChange={e=>setForm({...form,stock:+e.target.value})}/></div>
          <div><label>Ред</label><input type="number" value={form.sort_order} onChange={e=>setForm({...form,sort_order:+e.target.value})}/></div>
        </div>
        <div><label>URL на снимката</label><input value={form.image_url} onChange={e=>setForm({...form,image_url:e.target.value})}/></div>
  ...
  return (
    <div className="form-card">
      ...
      <div className="cf">
        ...
        <div><label>Подзаглавие</label><input value={form.subtitle} onChange={e=>setForm({...form,subtitle:e.target.value})} placeholder="Безплатен наръчник"/></div>
        <div><label>Описание</label><textarea rows={3} value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></div>
        <div><label>📄 PDF URL (Google Drive, Dropbox, Cloudflare R2...)</label><input value={form.pdf_url} onChange={e=>setForm({...form,pdf_url:e.target.value})} placeholder="https://drive.google.com/file/..."/></div>
        <div><label>🖼️ Корица URL (по желание)</label><input value={form.cover_image_url} onChange={e=>setForm({...form,cover_image_url:e.target.value})} placeholder="https://...jpg"/></div>
        {form.cover_image_url && <img src={form.cover_image_url} alt="" style={{height:120,objectFit:'contain',borderRadius:8,background:'#f8fafb'}}/>}
        <div><label>Ред</label><input type="number" value={form.sort_order} onChange={e=>setForm({...form,sort_order:+e.target.value})}/></div>
        <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer'}}>
          <input type="checkbox" checked={form.active} onChange={e=>setForm({...form,active:e.target.checked})} style={{width:'auto'}}/>
        ...
        <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:10,padding:'12px 16px',fontSize:13,color:'#166534'}}>
          💡 PDF файлът може да е в Google Drive (вземи Direct Link), Dropbox, или Cloudflare R2. Страницата за сваляне ще е на: <strong>/naruchnik/{form.slug||'slug'}</strong>
        </div>
        <div style={{display:'flex',gap:10,marginTop:8}}>
          <button className="btn-cancel" onClick={onCancel}>← Назад</button>
          <button className="btn-save" onClick={onSave} disabled={saving}>{saving?'Запазва...':'✓ Запази'}</button>
