...
// Пълно управление на собствени продукти + разфасовки (product_variants)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [pRes, vRes] = await Promise.all([
        ...
        // ?admin=1 → зарежда ВСИЧКИ варианти (включително неактивните)
  ...
  const save = async () => {
    ...
    try {
      ...
      await Promise.all(editVars.map(async (v, i) => {
        ...
        if (!v.id) {
          // New variant — POST (route ще махне id автоматично)
      ...
      {editing && (
        <div style={{ background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 16, padding: 22, maxHeight: '90vh', overflowY: 'auto', position: 'sticky', top: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* ── ЦЕНА (базова — само ако няма variants) ── */}
