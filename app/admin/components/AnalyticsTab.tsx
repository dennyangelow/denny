'use client'
// app/admin/components/AnalyticsTab.tsx — v5 с date range picker, comparison, export
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '16px 20px', flex: 1, minWidth: 140 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        {trend !== null && (
          <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: trend >= 0 ? '#dcfce7' : '#fee2e2', color: trend >= 0 ? '#15803d' : '#dc2626' }}>
...
function exportChart(containerId: string, filename: string) {
  ...
  import('html2canvas').then(({ default: html2canvas }) => {
    ...
    // html2canvas not available — fallback to SVG download
...
const Card = ({ id, title, children, onExport }: { id?: string; title: string; children: React.ReactNode; onExport?: () => void }) => (
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
      ...
      {onExport && (
        <button onClick={onExport}
          style={{ fontSize: 11, color: '#9ca3af', background: 'none', border: '1px solid #e5e7eb', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
          ↓ PNG
        </button>
      )}
    </div>
