'use client'
// app/admin/components/AnalyticsTab.tsx — v9 с уникални посещения
...
export function AnalyticsTab({ analytics, pageViews, orders }: Props) {
  ...
  const [pvMode, setPvMode] = useState<'total' | 'unique'>('total')
  ...
  const totalUnique = (pageViews as any)?.last30Unique || 0
  ...
  const funnelData = [
    ...
    { name: 'Уникални', value: totalUnique, pct: totalViews ? Math.round(totalUnique / totalViews * 100) : 0, color: '#8b5cf6' },
    { name: 'Поръчки', value: totalOrders30, pct: totalViews ? Math.round(totalOrders30 / totalViews * 100) : 0, color: '#16a34a' },
  ]

  // Page views daily — slice last N days, поддържа и unique
  const pvChartData = (() => {
    const raw = pageViews?.dailyChart?.slice(-Number(period)) || []
    return raw.map(d => ({
      date: d.date,
      count: d.count,
      unique: (d as any).unique || 0,
    }))
  })()
    ...
    <div className="an-root">
      ...
      <div className="an-head">
        <div>
          ...
          <p className="an-sub">{orders.length} поръчки · {analytics?.total || 0} афилиейт клика · {pageViews?.total || 0} посещения ({(pageViews as any)?.unique || 0} уникални)</p>
        </div>
      ...
      <div className="an-summary">
        {[
          ...
          { label: 'Уникални (30 дни)', value: (pageViews as any)?.last30Unique || 0, sub: `Днес уникални: ${(pageViews as any)?.todayUnique || 0}`, color: '#8b5cf6' },
          ...
          { label: 'Мобилни', value: `${pageViews?.mobilePercent || 0}%`, sub: 'от всички посещения', color: '#f59e0b' },
        ].map((s, i) => (
      ...
      <div className="an-grid">
        {/* Page views chart — total vs unique toggle */}
        <div className="ac span2">
          <div className="ac-hd">
            <h2>Посещения на сайта</h2>
            <div style={{ display: 'flex', gap: 4, background: '#f3f4f6', borderRadius: 8, padding: 3 }}>
              <button className={`ptab${pvMode === 'total' ? ' ptab-a' : ''}`} onClick={() => setPvMode('total')} style={{ fontSize: 12, padding: '4px 10px' }}>Общо</button>
              <button className={`ptab${pvMode === 'unique' ? ' ptab-a' : ''}`} onClick={() => setPvMode('unique')} style={{ fontSize: 12, padding: '4px 10px' }}>Уникални</button>
            </div>
          </div>
                ...
                <AreaChart data={pvChartData} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
                  <defs>
                    <linearGradient id="pvg" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={pvMode === 'unique' ? '#8b5cf6' : '#0ea5e9'} stopOpacity={0.25} />
                      <stop offset="95%" stopColor={pvMode === 'unique' ? '#8b5cf6' : '#0ea5e9'} stopOpacity={0.02} />
                    </linearGradient>
                  </defs>
                  ...
                  <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 12 }} formatter={(v: number) => [v, pvMode === 'unique' ? 'Уникални' : 'Посещения']} />
                  <Area type="monotone" dataKey={pvMode === 'unique' ? 'unique' : 'count'} stroke={pvMode === 'unique' ? '#8b5cf6' : '#0ea5e9'} strokeWidth={2} fill="url(#pvg)" dot={false} activeDot={{ r: 4 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            {funnelData.map((f) => (
              <div key={f.name}>
                ...
                <div style={{ height: 28, background: '#f3f4f6', borderRadius: 8, overflow: 'hidden', position: 'relative' }}>
                  <div style={{ width: `${Math.max(f.name === 'Посещения' ? 100 : f.pct, 2)}%`, height: '100%', background: f.color, borderRadius: 8, opacity: .85, transition: 'width .6s' }} />
                  <span style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 12, fontWeight: 700, color: '#374151' }}>{f.pct}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Top pages */}
        <div className="ac">
          <div className="ac-hd"><h2>Топ страници</h2></div>
          {(pageViews?.topPages || []).length === 0 ? <div className="emp">Няма данни</div> : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {(pageViews?.topPages || []).map((p, i) => (
                <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 12, width: 140, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'monospace' }}>{p.name}</span>
                  <div style={{ flex: 1, height: 6, background: '#f3f4f6', borderRadius: 99, overflow: 'hidden' }}>
                    <div style={{ width: `${(p.count / ((pageViews?.topPages || [])[0]?.count || 1)) * 100}%`, height: '100%', background: PIE_COLORS[i % PIE_COLORS.length], borderRadius: 99 }} />
                  </div>
                  <span style={{ fontSize: 12, color: '#6b7280', minWidth: 32, textAlign: 'right' }}>{p.count}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <style>{`
        ...
        @media(max-width:1100px){ .an-summary { grid-template-columns: repeat(3,1fr); } }
        @media(max-width:700px){ .an-summary { grid-template-columns: repeat(2,1fr); } }
        ...
        .an-sc-val { font-size: 22px; font-weight: 800; color: var(--text); margin-bottom: 4px; }
        ...
        .ac-hd { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 16px; gap: 10px; flex-wrap: wrap; }
