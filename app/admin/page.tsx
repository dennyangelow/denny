'use client'
// app/admin/page.tsx — v5 auto-refresh
...
export default function AdminPage() {
  ...
  return (
    <>
      ...
      {error && (
        <div style={{ position:'fixed', top:0, left:0, right:0, zIndex:999, background:'#fef3c7', borderBottom:'2px solid #fde68a', padding:'10px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, fontFamily:"'DM Sans',sans-serif", fontSize:14 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, color:'#92400e' }}>
            <span>⚠️</span>
            <span><strong>Внимание:</strong> {error}</span>
          </div>
          <button onClick={fetchAll} style={{ background:'#92400e', color:'#fff', border:'none', borderRadius:8, padding:'6px 16px', cursor:'pointer', fontWeight:700, fontSize:13, fontFamily:'inherit' }}>
            Опитай пак
          </button>
        </div>
      )}
