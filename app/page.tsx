...
function normOwn(p: OwnProduct): NormOwn {
  return {
    ...
    img: p.image_url || p.img || '',
    desc: p.description || p.desc || '',
    priceLabel: `${Number(p.price).toFixed(2)} лв.`,
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      fetch('/api/own-products').then(r => r.ok ? r.json() : null),
    ]).then(([affRes, catRes, ownRes]) => {
      if (affRes.status === 'fulfilled' && affRes.value?.products?.length > 0) {
        setAffProducts(affRes.value.products.filter((p: AffProduct) => p.active).map(normAff))
      }
      if (catRes.status === 'fulfilled' && catRes.value?.links?.length > 0) {
        setCatLinks(catRes.value.links.filter((l: CatLink) => l.active).map(normCat))
      }
  ...
  const addToCart = (product: NormOwn) => {
    setCart(prev => {
      ...
      return [...prev, { id: product.id, name: product.name, price: product.price, qty: 1, image: product.img }]
    })
    setCartVisible(true)
  }
  ...
  return (
    <>
        ...
        <div className="hero-inner">
          <div className="hero-left">
            <div className="trust-badge">
              <img src={`${CDN}/687aa8144659d_504368576_24540238958894103_5234342802938640767_n.jpg`} alt="Denny" style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,0.5)', flexShrink: 0 }} />
            ...
            <p className="hero-desc">Искаш едри, здрави и сочни домати — без болести, без гниене и без загубена реколта? Открий проверените методи и продукти.</p>
          ...
          <div className="hero-right">
            <div className="profile-card hero-float-anim">
              ...
              <div style={{ color: '#86efac', fontSize: 12, fontWeight: 700, marginBottom: 14 }}>Агро консултант &amp; фермер</div>
      ...
      <div className="trust-strip">
        {[{ i:'🌱', t:'Органични продукти' },{ i:'🚚', t:'Доставка до вратата' },{ i:'📞', t:'Лична консултация' },{ i:'⭐', t:'5-звездни отзиви' },{ i:'🔒', t:'Сигурно плащане' }].map(x => (
                ...
                >
                  <div style={{ position: 'relative' }}>
                    {p.img ? (
                      ...
                      <div style={{ width: '100%', height: 220, background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 64 }}>{p.emoji}</div>
                    )}
          ...
          <FadeIn>
            <div className="ginegar-inner">
              ...
              <div className="ginegar-img-wrap">
                ...
                <img src={`${CDN}/6940e17e0d4a3_pe-film-supflor-ginegar.jpg`} alt="Ginegar фолио" style={{ width: '100%', maxWidth: 280, borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.5)', position: 'relative' }} />
                <img src={`${CDN}/694242e9c1baa_ginegar-logo-mk-group.600x600.png`} alt="Ginegar logo" style={{ width: 90, marginTop: 20, filter: 'brightness(0) invert(1)', opacity: 0.6 }} />
              </div>
            </div>
          </FadeIn>
        </div>
      ...
      <section id="testimonials" className="section-wrap" style={{ background: '#fff' }}>
        ...
        <FadeIn>
          <div style={{ textAlign: 'center', padding: '36px 28px', background: 'linear-gradient(135deg, #f0fdf4, #dcfce7)', borderRadius: 24, border: '1px solid #bbf7d0', maxWidth: 700, margin: '48px auto 0' }}>
            <img src={`${CDN}/687aa8144659d_504368576_24540238958894103_5234342802938640767_n.jpg`} alt="Denny Angelow" style={{ width: 80, height: 80, borderRadius: '50%', objectFit: 'cover', border: '3px solid #16a34a', marginBottom: 18 }} />
      ...
      <section className="cta-section">
        ...
        <div style={{ maxWidth: 520, margin: '0 auto', position: 'relative', textAlign: 'center' }}>
          <FadeIn>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", color: '#fff', fontSize: 'clamp(26px, 4vw, 40px)', margin: '0 0 12px', fontWeight: 800 }}>Изтегли Безплатния Наръчник</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 32, marginBottom: 40, textAlign: 'left' }}>
            ...
            <div>
              ...
              {[{ label:'🌿 AgroApteki.bg', href:`https://agroapteki.com/${AFF}` },{ label:'🏡 Oranjeriata.bg', href:'https://oranjeriata.com/' },{ label:'🌱 AtlasAgro.eu', href:'https://atlasagro.eu/' }].map(l => (
                <a key={l.label} href={l.href} target="_blank" rel="noopener" style={{ display: 'block', marginBottom: 8, color: 'rgba(255,255,255,0.6)', textDecoration: 'none', fontSize: 14, fontWeight: 600, transition: 'color 0.2s' }}
            ...
            {!orderDone && cart.length > 0 && (
              <div style={{ padding: '16px 24px', borderTop: '1px solid #e5e7eb', background: '#fafaf8' }}>
                <button onClick={submitOrder} disabled={orderLoading || !orderForm.name || !orderForm.phone || !orderForm.address || !orderForm.city}
                  style={{ width: '100%', padding: '16px', borderRadius: 14, border: 'none', background: (!orderForm.name || !orderForm.phone || !orderForm.address || !orderForm.city) ? '#d1d5db' : '#16a34a', color: '#fff', fontWeight: 900, fontSize: 17, cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 6px 20px rgba(22,163,74,0.3)', fontFamily: 'inherit' }}>
...
const pageCSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .category-card{display:flex;align-items:center;gap:14px;background:#fff;border-radius:18px;padding:18px 20px;border:2px solid #e5e7eb;font-weight:700;font-size:14px;color:#1a1a1a;box-shadow:0 2px 12px rgba(0,0,0,.04);text-decoration:none;transition:all .25s}
  ...
  .cart-drawer{position:absolute;right:0;top:0;bottom:0;width:100%;max-width:480px;background:#fff;animation:fadeDown .3s ease;display:flex;flex-direction:column;box-shadow:-8px 0 48px rgba(0,0,0,0.2)}
