...
// ✅ НОВО — реални "🎉 [Име] току-що изтегли/поръча..." събития (leads +
// orders на собствени продукти), заменя randomDownloads()/SOCIAL_NAMES
// фалшивата логика, която живееше преди вътре в HandbooksPanel.tsx.
// ✅ getHandbookDownloadCounts() — реалният брой изтегляния по наръчник,
// преброен от leads (идентично на slugCounts в LeadsTab.tsx). ФИКС:
// naruchnici.downloads_count (полето, което четяхме досега директно от
// handbooks[i].downloads_count) се оказа фиктивно статично число (6800/
// 4200), напълно откъснато от реалните лийдове (2243/1331 в Email листа).
// naruchnici.downloads пък е непълен/скоро рестартиран брояч (4/2) —
// далеч под реалността. leads таблицата е единственият източник, който
// не може да бъде ръчно подправен.
import { getRecentActivity, getHandbookDownloadCounts } from '@/lib/social-proof'
...
const DEFAULT_SETTINGS: SiteSettings = {
  ...
  // ✅ {count} се заменя автоматично с реалния общ брой изтегляния (виж
  //    ctaSubtitleText в HomePage() по-долу) — същият патърн вече се
  //    ползва за seo_description. Ако редактираш това поле в админ панела,
  //    пиши {count} вместо твърдо число, за да остане вярно във времето.
  cta_subtitle:        'Над {count} фермери вече ги изтеглиха. Вземи **и двата безплатно** — тайните за едри домати и рекордни краставици.',
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  // ✅ ФИКС: същият проблем като в HomePage() — handbooks[i].downloads_count
  //    от базата е фиктивно число. SEO description-ът с {count} трябва да
  //    показва реалния брой, не 6 500/фиктивното, за да не подвежда Google
  //    и потребителите с различно число от това, което реално виждат в
  //    картата под наръчниците.
  const realDownloadCounts = await getHandbookDownloadCounts()
...
export default async function HomePage() {
  // ✅ НОВО: recentActivity се тегли успоредно с останалите данни — не
  //    забавя нищо друго (Promise.all), и ако leads/orders заявката
  //    гръмне по някаква причина, getRecentActivity() вътрешно си връща
  //    [] (виж lib/social-proof.ts catch-а), значи страницата пак се
  //    рендва нормално, просто без live ticker-а.
  const [
    {
      settings, atlasProducts, affiliateProducts, top6AffiliateProducts,
      categoryLinks, promoBanners, marketingSettings, testimonials, faq, faqCategories, handbooks: handbooksRaw, specialSections,
  ...
  // ✅ НОВО: cta_title/cta_subtitle бяха записвани в settings, но никога
  //    не се рендваха никъде на страницата — HandbooksPanel.tsx имаше свои
  //    ТВЪРДО закачени "Вземи Наръчника Безплатно"/"Над 6 000 фермери..."
  //    текстове, изцяло независими от админ панела. Сега реално се подават
  //    надолу. {count} в cta_subtitle се заменя с истинския totalDownloads
  //    (същият патърн като displayCount в generateMetadata по-горе) —
  //    старите текстове без {count} placeholder просто минават непроменени
  //    (String.replace на липсващ substring е no-op), значи нищо не се
  //    чупи за вече записано съдържание в базата, докато не добавиш {count}.
  const ctaTitle = settings.cta_title || DEFAULT_SETTINGS.cta_title
  const ctaSubtitleRaw = settings.cta_subtitle || DEFAULT_SETTINGS.cta_subtitle
  const ctaSubtitleText = ctaSubtitleRaw.replace(
    '{count}',
    totalDownloads > 0 ? totalDownloads.toLocaleString('bg-BG') : '6 500'
  )
        ...
        <div className="hero-inner">
          ...
          <div className="hero-right">
            <HandbooksPanel
              ...
              ctaTitle={ctaTitle}
              ctaSubtitle={parseBold(ctaSubtitleText)}
            />
          </div>
        </div>
      </section>
