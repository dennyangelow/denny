...
function UnsubscribeContent() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f9fafb', display:'flex', alignItems:'center', justifyContent:'center', padding:24, fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ background:'#fff', borderRadius:20, padding:'40px 36px', maxWidth:440, width:'100%', textAlign:'center', boxShadow:'0 8px 40px rgba(0,0,0,0.08)' }}>
        {status === 'done' ? (
          <>
          ...
          <>
            ...
            <p style={{ color:'#6b7280', fontSize:14, lineHeight:1.6, marginBottom:24 }}>
              {email ? <>Потвърди, че искаш да отпишеш <strong>{email}</strong> от имейл листата.</> : 'Невалиден линк за отписване.'}
            </p>
            {email && (
              <button
                onClick={handleUnsubscribe}
                disabled={status === 'loading'}
                style={{ width:'100%', padding:'14px', background:'#ef4444', color:'#fff', border:'none', borderRadius:12, fontWeight:800, fontSize:15, cursor:'pointer', fontFamily:'inherit' }}
              >
