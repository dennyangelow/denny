...
function CartProgressBar({ subtotal, freeShippingAbove, ms, fmt }: {
  ...
  return (
    <div style={{ background: done ? 'linear-gradient(135deg,#14532d,#166534)' : 'linear-gradient(135deg,#f0fdf4,#f7fef9)', border: done ? 'none' : '1.5px solid #bbf7d0', borderRadius: 14, padding: '12px 16px', marginBottom: 14, transition: 'all .4s cubic-bezier(.4,0,.2,1)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        {!done && <span style={{ fontSize: 12, color: '#6b7280', fontWeight: 600 }}>остават <strong style={{ color: '#16a34a' }}>{fmt(left)}</strong></span>}
      </div>
      <div style={{ background: done ? 'rgba(255,255,255,.15)' : 'rgba(255,255,255,.7)', borderRadius: 99, height: 8, overflow: 'hidden', boxShadow: 'inset 0 1px 3px rgba(0,0,0,.06)' }}>
        <div style={{ width: `${pct}%`, height: '100%', background: done ? 'linear-gradient(90deg,#4ade80,#86efac)' : 'linear-gradient(90deg,#16a34a,#4ade80)', borderRadius: 99, transition: 'width .6s cubic-bezier(.4,0,.2,1)', boxShadow: done ? 'none' : '0 0 8px rgba(22,163,74,.4)' }} />
      </div>
    </div>
  )
}
...
function OfferCard({ offer, products, onAddToCart, fmt, cartItems }: {
  ...
  useEffect(() => {
    if (alreadyInCart) setWasAdded(true); else setWasAdded(false)
  }, [alreadyInCart])
              ...
              <div style={{ background: '#f9fafb', borderRadius: 14, padding: 16, marginBottom: 16, display: 'flex', gap: 14, alignItems: 'center', border: '1px solid #f1f5f9' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ marginTop: 5, display: 'flex', alignItems: 'center', gap: 7 }}>
                    ...
                    {offer.discount_pct ? <><span style={{ fontSize: 13, color: '#9ca3af', textDecoration: 'line-through' }}>{fmt(variant.price)}</span><span style={{ fontSize: 10, fontWeight: 800, background: '#fee2e2', color: '#dc2626', padding: '2px 7px', borderRadius: 6 }}>-{offer.discount_pct}%</span></> : null}
                  </div>
                </div>
              </div>
            )}
...
function CartItemRow({ item, onUpdateQty, onRemove, fmt }: {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, padding: '13px 0', borderBottom: '1px solid #f1f5f9', alignItems: 'center' }}>
      <div style={{ width: 58, height: 58, flexShrink: 0, borderRadius: 13, overflow: 'hidden', background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e2e8f0' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' as const }}>
          ...
          {item.comparePrice > item.price && <>
            <span style={{ fontSize: 11.5, color: '#9ca3af', textDecoration: 'line-through' }}>{fmt(item.comparePrice * item.qty)}</span>
...
function CartDrawer({
  ...
  const { fmt, fmtLiter } = makeFmt(sym)
  ...
  return (
    <>
      <style>{`
        ...
        /* ══ OVERLAY ══════════════════════════════════════════════ */
        .cart-overlay {
          position: fixed; left: 0; right: 0; bottom: 0; top: 60px;
          background: rgba(15,23,42,.5); z-index: 999998;
        ...
        .cart-header {
          ...
          border-bottom: 1.5px solid #f1f5f9; background: #fff;
        ...
        /* ══ SCROLL AREA ══════════════════════════════════════════
           overscroll-behavior:contain — спира scroll на фона
           touch-action:pan-y — Safari scroll fix
           ══════════════════════════════════════════════════════════ */
        .cart-inner {
          flex: 1; overflow-y: auto; -webkit-overflow-scrolling: touch;
          overscroll-behavior: contain; overscroll-behavior-y: contain;
          padding: 12px 16px; min-height: 0; touch-action: pan-y;
        }
        ...
        @media (max-width: 640px) {
          .cart-footer { padding-bottom: max(24px, env(safe-area-inset-bottom, 24px)); }
        }
        ...
        @media (max-width: 640px) { .cart-input { font-size: 16px; } }
        .cart-input:focus { border-color: #16a34a; box-shadow: 0 0 0 2.5px rgba(22,163,74,.1) }
        ...
        .cart-btn-primary:hover:not(:disabled) { filter:brightness(1.06); box-shadow:0 6px 20px rgba(22,163,74,.4) }
        .cart-btn-primary:active:not(:disabled) { transform:scale(.98) }

        .cart-btn-secondary {
          width: 100%; padding: 11px; background: #f8fafc; color: #64748b;
          border: 1px solid #f1f5f9; border-radius: 12px; font-weight: 700;
          font-size: 13.5px; cursor: pointer; font-family: inherit; margin-top: 7px;
        ...
        .cart-close-btn {
          ...
          border-radius: 11px; cursor: pointer; font-size: 16px; color: #475569;
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 800, fontSize: 14.5, color: '#0f172a', letterSpacing: '-.01em', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                ...
                {!done && step === 'checkout' && <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>Стъпка 2 от 2</div>}
              </div>
            </div>
          ...
          <>
            ...
            {items.length > 0 && (
              <div className="cart-footer">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 13, fontSize: 21, fontWeight: 900, color: '#0f172a', letterSpacing: '-.02em' }}>
                  <span>Общо</span>
                  <div style={{ textAlign: 'right' as const }}>
        ...
        /* ── СТЪПКА 2: CHECKOUT ── */
          ...
          <>
            <div className="cart-inner">
              {/* COD banner */}
              <div style={{ background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', border: '1px solid #bbf7d0', borderRadius: 11, padding: '9px 13px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 9 }}>
                <span style={{ fontSize: 20, flexShrink: 0 }}>💵</span>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 12.5, color: '#15803d' }}>Наложен платеж</div>
                  <div style={{ fontSize: 11, color: '#166534', marginTop: 1 }}>Плащаш само при получаване — без предплащане</div>
                </div>
              </div>
              ...
              {error && (
                <div style={{ background: '#fee2e2', color: '#dc2626', borderRadius: 11, padding: '11px 14px', fontSize: 13, marginBottom: 14, display: 'flex', gap: 9, alignItems: 'flex-start', lineHeight: 1.5, border: '1px solid #fecaca' }}>
                  <span style={{ flexShrink: 0, fontSize: 15 }}>⚠️</span><span>{error}</span>
                </div>
              )}

              {/* Резюме */}
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden', marginTop: 6, marginBottom: 4 }}>
                <div style={{ padding: '11px 15px 10px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 7 }}>
                  <span style={{ fontSize: 14 }}>📋</span>
                  <div style={{ fontSize: 11, fontWeight: 800, color: '#64748b', letterSpacing: '0.07em', textTransform: 'uppercase' as const }}>Резюме на поръчката</div>
                </div>
                <div style={{ padding: '9px 15px' }}>
                  {items.map((i, idx) => (
                    <div key={i.variantId} style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: idx < items.length - 1 ? 9 : 0, marginBottom: idx < items.length - 1 ? 9 : 0, borderBottom: idx < items.length - 1 ? '1px solid #edf0f4' : 'none' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12.5, fontWeight: 700, color: '#0f172a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{i.productName}</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 2, flexWrap: 'wrap' as const }}>
                          <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500 }}>{i.variantLabel} · ×{i.qty}</span>
                          {i.fromOffer && (
                            <span style={{ fontSize: 9, fontWeight: 800, color: '#7c3aed', background: '#f5f3ff', border: '1px solid #ede9fe', padding: '1px 6px', borderRadius: 99, whiteSpace: 'nowrap' as const }}>
                      ...
                      <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 900, color: '#16a34a' }}>{fmt(i.price * i.qty)}</div>
                        {i.comparePrice > i.price && <div style={{ fontSize: 11, color: '#cbd5e1', textDecoration: 'line-through', marginTop: 1 }}>{fmt(i.comparePrice * i.qty)}</div>}
                      </div>
                    </div>
                  ))}
                </div>
                <div style={{ background: '#fff', padding: '11px 15px', borderTop: '1px solid #e2e8f0' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #e2e8f0', paddingTop: 10 }}>
                    <span style={{ fontWeight: 800, fontSize: 14.5, color: '#0f172a' }}>Общо</span>
                    <div style={{ textAlign: 'right' as const }}>
                      <div style={{ fontWeight: 900, fontSize: 22, color: '#16a34a', lineHeight: 1, letterSpacing: '-.02em' }}>{fmt(total)}</div>
                      {totalSavings > 0 && <div style={{ fontSize: 10.5, color: '#dc2626', fontWeight: 700, marginTop: 2 }}>Пести {fmt(totalSavings)} спрямо редовна цена</div>}
                    </div>
                  </div>
                </div>
              </div>
